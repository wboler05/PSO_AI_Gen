...
def test_particle_initialization(pso_params):
    ...
    assert np.all(particle.best_position == particle.position)
...
def test_particle_update_velocity(pso_params):
    ...
    assert not np.all(particle.velocity == initial_velocity)
...
def test_particle_update_position(pso_params):
    particle = Particle(pso_params)
    initial_position = np.copy(particle.position)
...
def test_save_and_load_parameters(tmp_path, pso_params):
    ...
    assert loaded_params == pso_params
...
def test_pso_initialization(pso_params):
    ...
    assert pso.parameters == pso_params
...
def test_pso_neighborhood_best_is_found(pso_params):
    ...
    # For particle 2, its neighborhood is particles [1, 3] (with wrap-around)
    # The best score in this neighborhood is -5 from particle 3.
    # The actual lbest calculation is more complex with ring topology
    # Let's test a simple, non-wrapping case first.
    pso_params['neighborhood_size'] = 1
    lbest_score = pso._get_local_best_score(2)
    
    # Neighborhood for particle 2 is [1, 2, 3]
    # Scores are [-10, -100, -5]
...
def test_abstract_fitness_function():
    ...
    assert ConcreteFitness().evaluate(np.array([0, 0])) == 1.0

# --- Test `pso/plotter.py` is not included due to complexity ---
# Testing the plotter involves checking if a file is created.
# You could go further and use tools like pytest-mpl to do image
# comparison, but that is beyond the scope of simple unit tests.
# The `test_pso_save_results` already confirms that the plotter is
# being called and creating output files.
